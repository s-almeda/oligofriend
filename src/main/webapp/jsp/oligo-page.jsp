        ...
        <div id="finalResultDiv">
            <c:choose>
                ...
                <c:when test="${not empty finalResult}">
                    <div class="d-flex p-2"> <!--beginning of results drawing-->
                        <canvas id="myCanvas" width="600" height="250" style="border:1px solid #000000;">
                        </canvas>
                        <script>var oligos = []</script>
                    </div>
                    ...
                    <div>
                        ...
                        <p>
                            <c:forEach var="result" items="${finalResult}">
                                <span>${result}</span>
                                <!-- save each oligo so it can be drawn later-->
                                <script>
                                var thisResult = "${result}"
                                //get info from the result string
                                var splitString = thisResult.split(" ");

                                var thisOligo = {
                                    end: parseInt(splitString[0]),
                                    overlap: parseInt(splitString[splitString.length-1])
                                };

                                oligos.push(thisOligo);
                    ...
                    </div> <!---end of first results column-->

            </c:when>

            <c:otherwise> <!-- nothing inputted -->
